"""프레임 repository — t_frame_adv 접근 (판독 대상 선정·변화 마킹).
...
# **이 컬럼은 불리언이 아니라 '검출된 주요 항목 개수'다(0~5).** 타입이 tinyint(1) 이라
# 스키마만 봐서는 안 보인다. `= 1` 로 필터하면 "1종만 검출된" 프레임만 잡아 대부분을
...
class FrameRepo:
    """t_frame_adv 접근 객체 — 판독 대상 조회·is_changed 마킹."""
    ...
    async def fetch_targets(self, v_id: int) -> list[tuple[int, int]]:
        ...
        """
        sql = (f"SELECT a.idx, a.idx_sec FROM t_frame_adv a "
    ...
    async def reset_is_changed(self, v_id: int) -> int:
        ...
        async with self._db.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "UPDATE t_frame_adv SET is_changed = 0 WHERE v_id = %s", (v_id,))
                n = cur.rowcount
    ...
    async def mark_is_changed(self, v_id: int, idxs: list[int]) -> int:
        ...
        async with self._db.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    f"UPDATE t_frame_adv SET is_changed = 1 WHERE v_id = %s AND idx IN ({ph})",
                    (v_id, *idxs))
                return cur.rowcount
    ...
    async def count_changed(self, v_id: int) -> int:
        ...
        async with self._db.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT COUNT(*) FROM t_frame_adv WHERE v_id = %s AND is_changed = 1",
                    (v_id,))
